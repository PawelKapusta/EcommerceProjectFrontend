...
const OrderScreen = () => {
  ...
  const onSubmit = async data => {
    ...
    if (items.length === 0 || totalPrice === 0 ) {
      enqueueSnackbar(`Please add some products, because your basket is empty! Go to basket now! `, { variant: "error", autoHideDuration: 10000 });
    } else {
      const order = {};
      const allItems = [];
      order.UserID = userID;
      ...
      createOrder(order)
       .then(res => {
         setStatus(res?.status);
          res?.status === 200 ? navigate(`/payment/${res?.data?.ID}`) : "";
         clearBasket();
       }).catch(error => {
